    ...
    .then((user) => {
      ...
      bcrypt
        ...
        .then((valid) => {
          ...
          res.status(200).json({
            userId: user.id,
            token: jwt.sign({ userId: user.id }, "process.env.DB_TOKEN", {
              expiresIn: "24h",
            }),
...
exports.getAllAccounts = (req, res, next) => {
    //: AJOUT => {where: { firstname: req.params.firstname } 
  User.findAll({where: { firstname: req.params.firstname },
  }).then((users) => res.status(200).json(users));
  console.log(User).catch((error) => res.status(400).json({ error }));
};
